...
export function UploadZone({ modelStatus, loading, preprocess, onTogglePreprocess, onFile }: Props) {
  const disabled = modelStatus.status !== 'ready' || loading;
  ...
  return (
    <section id="recognize" className="rounded-3xl border border-dashed border-blue-300 bg-white p-8 shadow-sm dark:border-blue-900 dark:bg-slate-900">
      <div {...getRootProps()} className={`flex flex-col items-center justify-center rounded-2xl px-4 py-10 text-center transition ${isDragActive ? 'bg-blue-50 dark:bg-blue-950/40' : 'bg-slate-50 dark:bg-slate-950'}`}>
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">支持 JPG / PNG / WebP，最大 10MB。识别全程在本地服务器完成。</p>
        ...
        {modelStatus.status !== 'ready' && <p className="mt-4 text-sm text-amber-600 dark:text-amber-300">{modelStatus.message}</p>}
      </div>
    </section>
  );
}
